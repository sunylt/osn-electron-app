...
let scene: null | osn.IScene = null;
...
export function initOBS() {
  console.debug('Initializing OBS...', uuid());
  console.log(fixPathWhenPackaged(path.join(app.getAppPath(), 'node_modules', 'obs-studio-node')))
  osn.IPC.host(`${uuid()}`);
  osn.NodeObs.SetWorkingDirectory(fixPathWhenPackaged(path.join(app.getAppPath(), 'node_modules', 'obs-studio-node')));

  const obsDataPath = fixPathWhenPackaged(path.join(app.getAppPath(), 'osn-data')); // OBS Studio configs and logs
...
export function getCameraSource() {
...
export function setupPreview(window, bounds) {
  if (!scene) return;
  console.log('setupPreview>>>>', bounds)
...
export function resizePreview(window, bounds) {
...
export function getAvailableValues(category, subcategory, parameter) {
...
export function getNextSignalInfo() {
...
export function busySleep(sleepDuration) {
